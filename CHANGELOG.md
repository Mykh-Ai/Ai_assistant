...
## Unreleased
- Added production-safe local legacy multi-workspace migration tooling with redacted read-only planning, deterministic ownership mapping, logical fingerprint pinning, exclusive-lock refusal, verified SQLite and content-hashed storage backup, separately audited target construction, atomic apply, manifest-bound rollback, emergency restore, CLI modes, and fixture coverage. The real server DB and Docker restart remain blocked until the new server dry-run report is presented and explicitly approved.
