...
## Unreleased
- Fixed Product Truth admin-review offers so non-supported customization-allowed capability answers open the existing request preview with standard Schváliť / Upraviť / Zrušiť buttons instead of sending buttonless guidance.
- Polished InfoHelp/admin-review offers so actionable Product Truth or triage offers enter the existing confirmation-gated request preview with shared Schváliť / Upraviť / Zrušiť decision buttons and DecisionResolver handling; no request is saved before approval.
