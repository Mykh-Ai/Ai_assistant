...
- Repaired the bounded OfficeFlow Android real-device PDF path after production read-only evidence showed the Samsung request reached the API but received 404 before Android could download or render bytes. The persisted legacy numeric-owner PDF exists in the shared API/Telegram mount, is size-bounded and `%PDF-` valid; the Stage A resolver rejected it solely because the owner now has two workspaces. A legacy file may now stream only when its persisted invoice row, requested workspace, invoice number, filename, and pointer are unique; duplicate-number/shared-pointer, flat/arbitrary, foreign, oversized, and invalid-signature paths still fail closed. Android keeps all existing PDF safety gates and now distinguishes unavailable, network, invalid-response, and rendering failures without exposing internals. No route, schema, migration, path rewrite, production deployment/write, Cloudflare, Telegram/FSM, or new capability was added; backend deployment and Samsung re-test remain required, so C1 stays `runtime_not_proven`.

- Stabilized controlled OfficeFlow Android pilot APK signing so future pilot builds can be installed in place without losing the AndroidKeyStore-backed session. Pull-request builds remain secret-free and use their ordinary debug signer; only manual runs on the exact pilot branch can enter the required-reviewer `android-pilot-signing` environment. The pilot workflow restores the protected keystore only in runner-temporary storage, passes credentials through environment variables, pins and verifies public certificate SHA-256 `3835035c2df22b22406a00c359cc1a03e54f61852c302ed7c6392702a9d8e6fe`, publishes endpoint/signing evidence, and cleans up on every exit. Gradle accepts only a complete signing input set. One final uninstall/install/enrollment is still required when moving from the earlier ephemeral-debug signer; subsequent same-signer updates preserve app data. No private key/credential entered Git, and no product capability, API, server, Cloudflare, database, Telegram, or enrollment-protocol behavior changed.

- Implemented approved OfficeFlow Android C1 Home Top-Level Navigation V2 on the existing Stage B pilot branch. The app now starts at Home after session/workspace resolution, shows exactly `Faktúry`, `Bločky`, `Kontakty`, `Pracovný čas`, `Analytika`, and `InfoHelp`, keeps `Hlas / Chat` separate and unavailable, removes the flat invoice/contact bottom navigation, and exposes only existing invoice/contact reads while every other sublevel remains a zero-effect local unavailable state. Responsive scrolling/inset/long-text layouts and bounded detail retry/back states were added. The real-phone invoice-detail failure was traced to valid production-shaped `invoice_item.unit = null`; the Android decoder now accepts that nullable field and renders a safe fallback without any backend/schema/route change. Product Truth/InfoHelp now record the active controlled HTTPS pilot and bounded Samsung evidence while keeping C1 `partial` until the new APK proves Home, repaired detail/PDF, restart, profile switching, font-scale/back behavior, and logout. No Android mutation, AI/STT, Telegram/FSM, DB, server, Cloudflare, enrollment, or deployment change was made.
- Implemented the approved OfficeFlow Android Read-Only Shell V1 Stage B as a one-module Kotlin/Jetpack Compose thin client over the nine existing Stage A API routes. The app supports administrator-code enrollment, AndroidKeyStore AES/GCM session storage in no-backup app-private storage, serialized refresh rotation with an explicit ambiguous-response stop state, local-only validated workspace selection, read-only outgoing invoice list/detail/PDF and contact screens, and truthful online/offline sign-out. Release configuration is HTTPS-only and fails without an endpoint; the sole debug cleartext exception is `10.0.2.2`. Added deterministic JVM/instrumentation tests, a reproducible Gradle wrapper, Android build documentation, and a pull-request/workflow-dispatch debug-APK artifact workflow. Product Truth/Slovak InfoHelp now expose only `first_party_android_client=partial`, administrator/setup-required, and explicitly state that the production API is not deployed. The small Stage A compatibility repair returns bounded 423 for temporary block so valid credentials can resume after approval, while revoked/expired/deleted access remains terminal 401. No route/schema/business mutation/Telegram FSM/AI capability, production migration, API deployment, server change, Play Store release, or claim of live phone availability was added.
...
- Fixed Gmail OAuth refresh failure classification. Google Auth `RefreshError`
  ...
  workspace binding/grant `needs_reauth` and direct the administrator through
  `/gmail_connect`. Retryable Google Auth refresh/transport failures remain
...
- Restored the pre-PR63 unknown-input and InfoHelp route while preserving unrelated runtime issue, agent-claim, Gmail, invoice, contact, document, work-time, Drive, profile, access-control, and server work.
- No database migration, schema change, storage rewrite, or business-data modification is part of this rollback.
- Historical PR #63/#64 design, automated, deployment, and failed-interactive evidence remains in the task-specific architecture and acceptance artifacts, marked superseded and rolled back.
...
- Fixed invoice analytics for explicit noisy/Cyrillic customer references: a bounded extractor returns only the stated name, Python reuses the invoice-generation exact/normalized/confirmed-alias/fuzzy chain before a bounded fallback, prefilters by trusted `contact_id`, and fails safe when unresolved; no contact alias, invoice, PDF, or business data is changed.
## Unreleased
- Added a disabled-by-default Gmail gmail.readonly OAuth foundation and workspace-scoped bank-statement attachment collector with hashed single-use OAuth state/nonce, encrypted tokens, bounded overlap, atomic storage and deduplication, `needs_reauth` recovery, admin commands, bounded notifications, and optional enqueue through the existing owner Google Drive archive. Parsing, reconciliation, Gmail mutation, real Google verification, callback deployment, and production smoke remain unproven.
