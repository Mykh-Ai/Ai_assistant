...
- Fixed invoice analytics for explicit noisy/Cyrillic customer references: the bounded resolver can select only from current tenant contacts or `unknown`, Python prefilters by trusted `contact_id`, the planner cannot reapply a raw-name filter, and no contact alias, invoice, PDF, or business data is changed.
## Unreleased
