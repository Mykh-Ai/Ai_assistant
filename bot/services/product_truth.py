...
_REGISTRY: tuple[ProductTruthCapability, ...] = (
    ...
    _capability(
        ...
        status=ProductTruthStatus.UNSUPPORTED,
        summary_for_user='Runtime customization request drafting/storage is not implemented.',
        current_limitations=('The bot must not claim a request was created or saved.',),
        truth_source_refs=('docs/Customization_Request_Layer.md', 'docs/Product_Truth_Layer.md'),
        safe_next_steps=('Do not offer saved customization requests until the request layer exists and is confirmation-gated.',),
        forbidden_claims=('I created a support request.', 'Your customization request has been saved.'),
    ),
    _capability(
    ...
    _capability(
        ...
        domain='info_help',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='Current runtime has deterministic Level 1 unknown-input guidance only.',
        current_limitations=('Level 2 capability-aware Product Truth Q&A is not implemented in this patch.',),
        runtime_owner='bot/services/info_help.py::build_top_level_unknown_guidance',
        linked_handlers=('bot/services/info_help.py',),
        truth_source_refs=('docs/Info_Help_Guidance_Layer.md', 'PROJECT_LOG.md'),
        test_refs=('tests/test_invoice_intent_prerouter.py', 'tests/test_voice_state_routing.py'),
        safe_next_steps=('Keep Level 1 fallback honest; wire Product Truth consumption only in a later InfoHelp patch.',),
        forbidden_claims=('InfoHelp Level 2 is implemented.', 'I answered from Product Truth in the live bot.'),
        notes_for_agents='This registry foundation does not make InfoHelp capability-aware at runtime.',
    ),
)
