...
_REGISTRY: tuple[ProductTruthCapability, ...] = (
    ...
    _capability(
        ...
        domain='invoices',
        status=ProductTruthStatus.SUPPORTED,
        summary_for_user='Starts the bounded edit flow for an already persisted outgoing invoice after supplier-scoped lookup, with direct field buttons and bounded text/voice operation selection.',
        current_limitations=(
            'Exact invoice references, invoice numbers, item numeric values, service values, and final descriptions remain precision-sensitive/text-first; invoice dates may use bounded voice normalization followed by Python validation.',
        ),
        runtime_owner='bot/handlers/invoice.py::process_invoice_text',
        ...
        safe_next_steps=('Ask for a specific invoice reference, then choose a direct edit button or name the exact field by text/voice.',),
        requires_setup=True,
        setup_state_keys=('authorized_user', 'supplier_profile'),
    ),
    _capability(
    ...
    _capability(
        ...
        domain='gmail',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='A configured workspace can connect one verified Gmail account, collect matching bank-statement attachments, and determine the archive month for supported PDF layouts.',
        current_limitations=(
            ...
            'A supported encrypted PDF can be opened in memory with one administrator-managed file-backed opening password. The owner/edit password is not used, the decrypted content is not stored, and an ambiguous period is withheld from Drive rather than guessed.',
            'The archive month is the calendar month covering the greatest number of days in the validated statement interval; this is period routing, not transaction parsing.',
        ...
        test_refs=(
            ...
            'tests/test_gmail_statement_period.py',
        ),
        ...
        forbidden_claims=(
            ...
            'The bot can edit a bank PDF, recover its owner password, or decrypt arbitrary bank layouts.',
            'A statement with an unknown period was safely archived under its import month.',
    ...
    _capability(
        ...
        domain='contacts',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='Supports confirmation-gated manual/document contact intake, deterministic official Slovak company search by name or IČO when enabled for the active workspace, and an optional confirmation-gated 14-day registry monitor that can group the same owner\'s duplicate company contacts by canonical IČO.',
        current_limitations=(
            ...
            'The periodic monitor is separately disabled by default, requires an exact eight-digit IČO and active owner authorization, and may maintain saved contacts in inactive profiles. Duplicate contacts are grouped only for the same authorized owner, canonical IČO, and identical official target snapshot; one explicit proposal-button confirmation applies the explicitly grouped rows atomically.',
            'There is no commercial-registry scraping or automatic contact creation from idle attachments, and the monitor never rewrites issued invoices or PDFs.',
        ),
        ...
        linked_handlers=(
            ...
            'bot/handlers/contact_registry_monitor.py',
        ),
        truth_source_refs=(
            'docs/TZ_FakturaBot.md',
            'docs/Product_Truth_Layer.md',
        ...
        test_refs=(
            ...
            'tests/test_contact_registry_monitor.py',
            'tests/test_contact_registry_monitor_handler.py',
        ),
        forbidden_claims=(
    ...
    _capability(
        ...
        domain='work_hours',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='Records work-day opening at the current business time when no time is stated or at a preview-confirmed explicit arrival time, closes an open day by confirmed end time or net worked duration, supports manual time ranges/totals, applies a lunch-break deduction for reports, generates a monthly Excel report, and can delete one selected month after confirmation.',
        current_limitations=(
            ...
            'Explicit arrival times, manual ranges, duration-only totals, ambiguous extracted exact values, lunch-break updates, and monthly deletion require preview confirmation before save/delete.',
        ...
        safe_next_steps=(
            'Say zacinam pracovny den for the current time, zapis prichod o 7:10 for an explicit arrival, zatvor den o 17:00, zapis dnes 6 odpracovanych hodin, pracoval som dnes od 5:30 do 17:00, vytvor vykaz hodin za jun 2026, nastav obednu prestavku na 30 minut, or vymaz dochadzku za jul 2026.',
        ),
        customization_allowed=True,
        requires_setup=True,
        setup_state_keys=('authorized_user',),
