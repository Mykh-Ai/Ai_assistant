...
_REGISTRY: tuple[ProductTruthCapability, ...] = (
    ...
    _capability(
        ...
        domain='info_help',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='Current runtime has deterministic Product Truth-backed InfoHelp for selected topics plus bounded contextual recovery for primary unknowns, unknown slash commands, and active-FSM help.',
        current_limitations=(
            'This is a partial Level 2 foundation with deterministic fast-path, bounded triage, and one no-retry contextual recovery call; it is not complete arbitrary capability-aware Q&A.',
            'Recent context is process memory only, limited to three user plus three bot turns with a ten-minute TTL, and is lost on restart.',
            'The recovery model returns bounded classification metadata only; Python validates IDs and owns wording, callbacks, FSM transitions, Product Truth, confirmations, and every side effect.',
            'Active-FSM recovery never switches directly into a fresh business flow, and no context is reconstructed from logs or persisted history.',
            'Full Level 2 still requires broader capability coverage and deployed multilingual/noisy Telegram acceptance.',
        ),
        runtime_owner='bot/services/info_help.py plus bot/services/contextual_info_help_recovery.py and bot/handlers/info_help_recovery.py',
        linked_handlers=('bot/services/info_help.py', 'bot/handlers/info_help_recovery.py', 'bot/services/active_fsm_guard.py'),
        truth_source_refs=('docs/Info_Help_Guidance_Layer.md', 'docs/architecture/INFOHELP_CONTEXTUAL_RECOVERY_V1_ARCHITECTURE_DESIGN_PROOF.md', 'PROJECT_LOG.md'),
        test_refs=('tests/test_product_truth.py', 'tests/test_info_help.py', 'tests/test_contextual_info_help_recovery.py', 'tests/test_active_fsm_contextual_help.py'),
        safe_next_steps=(
        ...
        forbidden_claims=(
            ...
            'InfoHelp has complete voice/STT parity across every product capability.',
        ),
        notes_for_agents='Partial Product Truth-backed runtime, bounded triage, contextual recovery, unknown-command recovery, and active-FSM help exist locally; do not describe InfoHelp as complete Level 2 or production-accepted.',
