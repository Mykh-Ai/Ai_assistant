...
_REGISTRY: tuple[ProductTruthCapability, ...] = (
    ...
    _capability(
        ...
        current_limitations=(
            ...
            'No email, SMS, accounting export, or bank matching is performed. Google Drive archive is only a separate owner-run service-account integration when configured.',
        ),
    ...
    _capability(
        ...
        current_limitations=(
            ...
            'When owner-run Google Drive service-account mode is enabled, the action may enqueue the local PDF for Drive archive; otherwise it records only the local Drive archive stub.',
        ),
        ...
        notes_for_agents='Supported MVP is manual confirmation-gated bot-local payment state only. Drive archive is optional owner-run service-account enqueue/upload and never bank matching.',
    ),
    _capability(
    ...
    _capability(
        ...
        domain='google_drive',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='Owner-run Google Drive archive is available as a partial service-account integration for configured deployments.',
        current_limitations=(
            'This is not per-client OAuth or SaaS Drive sync; it uses one owner-managed service account and one manually shared Drive root folder.',
            ...
            'Missing credentials, root folder access, or Google API dependencies leave jobs retryable/failed without deleting local files.',
        ),
        runtime_owner='bot/services/invoice_drive_archive_service.py and bot/services/google_drive_archive_scheduler.py',
        ...
        safe_next_steps=('Configure GOOGLE_DRIVE_ENABLED=1, service-account JSON path, and a root folder id shared with the service account before expecting uploads.',),
        customization_allowed=True,
        requires_setup=True,
        requires_admin=True,
        requires_external_credentials=True,
    ...
    _capability(
        ...
        domain='google_drive',
        status=ProductTruthStatus.PARTIAL,
        summary_for_user='After a due-date follow-up decision such as marking an invoice paid, a configured owner-run service-account setup can enqueue the invoice PDF for Google Drive archive.',
        current_limitations=(
            'The upload runs through the archive worker and requires GOOGLE_DRIVE_ENABLED=1, service-account JSON, and a root folder shared with the service account.',
            ...
            'This is not user OAuth, not SaaS multi-client Drive, and not bank-confirmed settlement.',
        ),
        ...
        forbidden_claims=(
            ...
            'This is per-user Google OAuth Drive storage.',
        ),
    ),
    _capability(
