...
_SLOVAK_CAPABILITY_COPY = {
    ...
    'google_drive_invoice_storage': {
        ...
        'limitation': 'Nie je to per-client OAuth ani všeobecná SaaS synchronizácia. Vyžaduje OAuth client credentials, GOOGLE_TOKEN_CRYPTO_SECRET, jednorazový owner OAuth bootstrap, encrypted refresh token, root folder id v osobnom My Drive a zapnutý worker. Service-account režim nie je podporovaný pre personal My Drive bez Workspace/Shared Drive. PDF vystavených faktúr, potvrdené bločky, prijaté faktúry a podporované bankové výpisy sa zaraďujú asynchrónne pod samostatný priečinok vlastníckeho business profilu; lokálne uloženie neznamená úspešný upload, metadata ostávajú lokálne a staré Drive súbory sa automaticky nepresúvajú. PDF faktúr ostáva lokálne v bote. Stav konkrétneho účtu sa musí overiť stavovým príkazom.',
