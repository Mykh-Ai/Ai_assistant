...
- Validation before remote Android CI: focused PDF/API suite `20 passed`; full repository regression `2617 passed, 7 subtests passed in 615.78s`; `python -m compileall -q bot` and `git diff --check` passed. The host has only Java 8, so the required JDK 17 Android JVM/lint/assemble proof is delegated to the existing GitHub workflow. Production changed: **NO**. Backend deployment required before device PDF re-test: **YES**, but not performed.
