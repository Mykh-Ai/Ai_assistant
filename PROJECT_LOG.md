...
- Reconfirmed the exact production defect read-only: the Samsung PDF GET reached `officeflow-api` and returned 404; DB integrity is `ok`; Telegram and API containers share `/bot/data/storage`; the persisted numeric-owner PDF exists in the API container, has the exact expected filename, is within 25 MB, and begins with `%PDF-`; the requested invoice belongs to the requested workspace; the owner has two invoice workspaces; its invoice number and persisted pointer each occur in exactly one row/workspace. No material variance from the source evidence was found.
- Narrowed only `WorkspaceInvoicePdfStorageService.resolve_existing_read_path()` legacy compatibility. A numeric-owner file is admitted only when the current invoice id/workspace is the sole row with that invoice number and no other owner invoice reuses the persisted pointer or expected filename. Exact workspace roots remain unchanged. Duplicate-number, shared-pointer/filename, foreign-workspace, flat/arbitrary, traversal/escape, missing, oversized, invalid-signature, and otherwise unprovable cases still fail closed as bounded not-found with no path disclosure.
- No PDF/path was moved, rewritten, generated, migrated, or exposed. No DB/schema, API route, Android, Telegram/FSM, AI/STT/LMM, Product Truth, InfoHelp, Cloudflare, deployment, or production state changed. AI maturity and self-learning are unchanged/not applicable. The active Stage A architecture checkpoint received one matching sentence update so its storage contract does not contradict runtime code.
- Added/retained backend regressions for unique two-workspace legacy success, duplicate invoice-number rejection, shared persisted-pointer rejection, foreign-workspace rejection, flat/outside/missing/invalid/oversized rejection, exact `application/pdf`, bounded valid content, and `Content-Length`. Focused API suite: `21 passed in 23.28s`. Full main-derived suite: `2607 passed, 7 subtests passed in 636.63s`; this is the recorded main baseline `2605` plus two new backend test functions. The higher `2617` count belongs to PR #105's unrelated C1 test additions and was intentionally not ported.
- PR #106 hardening added the explicit `test_legacy_pdf_root_rejects_same_filename_with_different_pointer` regression: a second invoice for the same owner in another workspace has a different persisted pointer string whose basename still collides with the requested invoice's expected filename, and the requested legacy PDF fails closed as sanitized `404 not_found`. The resolver and architecture wording remained unchanged. Focused API suite: `22 passed in 26.47s`; full suite: `2608 passed, 7 subtests passed in 637.51s`; `compileall` and `git diff --check` passed.
