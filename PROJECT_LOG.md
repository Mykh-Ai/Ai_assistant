# 2026-08-20 - OfficeFlow Android C1 real-device PDF failure repair

- Continued on `agent/officeflow-android-read-only-shell-v1` and Draft PR #105 from exact pre-change SHA `1f85318ce6ca27c709a307b7598c476fe7264f24`. Scope is only the existing invoice PDF read path and sanitized failure UX: no route, schema, navigation, capability, Telegram/FSM, AI/STT/LMM, Cloudflare, enrollment, business mutation, storage migration, production write, restart, or deployment is included. AI maturity and self-learning remain unchanged/not applicable.
- Read-only production evidence proved the authorized Samsung sent the expected workspace-scoped PDF GET and the deployed API returned HTTP 404. The production DB integrity check is `ok`; both `fakturabot` and `officeflow-api` use the same `/bot/data/storage` mount; the tested persisted file exists inside the API container, is within the 25 MB bound, and starts with `%PDF-`. Therefore Android download/temp/renderer code never received PDF bytes, and neither a missing mount nor an absent/corrupt artifact is the root cause.
- Diagnosed the exact resolver defect: the file is in the historical numeric-owner directory, and the actor now has two workspaces. Stage A rejected every numeric-owner path for a multi-workspace actor although the tested invoice number and persisted pointer each occur once and belong to the requested workspace. The repair admits only that narrowly proven case. The current row/workspace/invoice number/filename/pointer must remain unique; duplicate invoice numbers, shared pointers, flat/arbitrary paths, foreign workspace paths, unsafe size/signature, and path escape still fail closed. No persisted path or file is rewritten, moved, generated, or exposed.
- Android retains the 25 MB, `application/pdf`, `%PDF-`, app-private temporary storage, atomic partial-file rename, tenant scope, and cleanup gates. PDF failures now distinguish unavailable artifact, network outage, invalid response, and local `PdfRenderer` failure with bounded Slovak text; raw server/path details are never shown. Unknown/chunked content length remains safe because streaming is still capped at 25 MB.
- Added sanitized backend regression for the exact real-device shape: a unique legacy file remains readable after a second workspace is added, then fails closed when another workspace owns the same invoice number. Existing foreign/flat/missing protections remain covered. Added exact server `Content-Type: application/pdf` and `Content-Length` assertions, Android chunked/private-parent/partial-rename coverage, and JVM failure-classification coverage.
- Updated the Stage A/Stage B architecture and acceptance contracts plus the C1 real-device evidence. Current Samsung evidence now proves Home, invoice list/detail, profile switching, contacts, and expected unavailable domains; PDF remains failing until backend deployment and re-test. Overall verdict remains `runtime_not_proven`.
- Validation: focused PDF/API suite `20 passed`; full repository regression `2617 passed, 7 subtests passed in 615.78s`; `python -m compileall -q bot` and `git diff --check` passed. PR run `32395879671` passed JDK 17 Android JVM tests, lint, and debug assembly. Protected stable-pilot run `32396113507` passed the same gates, exact HTTPS endpoint proof, pinned-certificate proof, checksum, artifact upload, and signing-key cleanup on exact code/workflow SHA `4b4d249859865c6ab85344a31e7757bb6e7e28e7`. Artifact `officeflow-android-pilot-4b4d249859865c6ab85344a31e7757bb6e7e28e7` (id `9416786993`) was downloaded and its APK independently matched SHA-256 `8c41ceef7fd9ee2e641433fca80be82400f6fa3957dc9de7ced16cdbab3717b8`; workflow certificate SHA-256 matched the pinned `3835035c2df22b22406a00c359cc1a03e54f61852c302ed7c6392702a9d8e6fe`. Production changed: **NO**. Backend deployment required before device PDF re-test: **YES**, but not performed.

# 2026-08-20 - OfficeFlow Android stable pilot APK signing

- Continued on the existing `agent/officeflow-android-read-only-shell-v1` branch and Draft PR #105 from exact pre-change SHA `4b49f73b4e3bf7133b0c7484f94f9feba79e7e1a`. Scope is packaging/session continuity only: no Android capability, API route, Telegram/FSM, database, server, Cloudflare, enrollment protocol, or business behavior changed.
- Proved the repeated-enrollment cause by comparing two GitHub-built pilot APKs: their debug signing-certificate SHA-256 values differed, so Android correctly treated them as different signers and required uninstall, which erased the app-private AndroidKeyStore session. The server session itself remained active.
- Created one RSA-4096 long-lived controlled-pilot signing key outside the repository with a password-protected PKCS12 backup and a Windows-DPAPI-protected password backup. The private key/password were never written to Git or output; the public certificate SHA-256 is `3835035c2df22b22406a00c359cc1a03e54f61852c302ed7c6392702a9d8e6fe`.
- Created the protected GitHub environment `android-pilot-signing` with required-reviewer approval and an exact deployment-branch policy for the pilot branch. Loaded the keystore, store password, alias, and key password as four environment secrets without printing their values.
- Split the existing workflow into a secret-free pull-request job and a manual environment-gated pilot job. The pilot job uses the fixed approved HTTPS endpoint, restores the keystore only under the runner temporary directory, passes signing credentials only through environment variables, verifies the generated endpoint and pinned APK certificate, publishes checksum/evidence, and removes the restored keystore on every exit.
- Added optional all-or-none Gradle pilot signing. Ordinary local/PR debug builds retain their normal debug signer; a partial pilot-signing environment fails configuration. A local forced pilot assembly succeeded and `apksigner` returned the pinned certificate exactly. The first stable-signed APK requires one final uninstall/install/enrollment; later APKs signed with the same key can update in place and preserve AndroidKeyStore/app data unless the user clears data/uninstalls, changes device, or server/session policy requires enrollment.
- Added structural regression coverage and Android operator documentation. Product Truth and InfoHelp were intentionally unchanged because signing changes package provenance/update continuity, not the app's supported business capabilities. Merge, Play Store/release publication, production runtime writes, and any new enrollment issuance are out of scope.
- Validation: ordinary Android `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug` passed; forced stable-signed pilot assembly passed and `apksigner` matched the pinned certificate; focused signing/Stage B boundaries `10 passed`; full repository regression `2617 passed, 7 subtests passed`; `python -m compileall -q bot`, private-key tracked-file scan, and `git diff --check` passed.
- The first two remote attempts (`32384608881`, `32385213892`) failed closed at the signer-evidence parser after successful builds; artifact upload was skipped and the restored keystore cleanup passed. The gate was repaired to extract the public PEM certificate from the APK and hash its DER form rather than parse version-dependent human output.
- GitHub Actions run `32385806345` on exact code/workflow SHA `a19b45f7aa01d288410580b94a88202a09545c03` passed the environment approval, Android JVM tests, lint, stable-signed assembly, endpoint proof, PEM/DER signer pin, checksum, artifact upload, and always-run key cleanup. Artifact `officeflow-android-pilot-a19b45f7aa01d288410580b94a88202a09545c03` (id `9412994306`) was downloaded and independently verified: APK SHA-256 `52c8fd682d6f6696637df58937a0dc05c5e2a742f7db68249260e733699f818a`, exact approved HTTPS endpoint, emulator endpoint absent, and local `apksigner` matched the pinned certificate.

# 2026-08-20 - OfficeFlow Android C1 Home Navigation V2 implementation

- Continued on the existing `agent/officeflow-android-read-only-shell-v1` branch and Draft PR #105 at exact pre-change branch SHA `44098e5b165037a1974214905aa2eb8534e83283`; the approved C1 architecture proof verdict was `ready_for_handoff` and design variance remained `none_identified`.
- Implemented the frozen Home architecture: session/workspace resolution now opens `Domov`; the exact six business domains are `Faktúry`, `Bločky`, `Kontakty`, `Pracovný čas`, `Analytika`, and `InfoHelp`; `Hlas / Chat` is separate. The flat invoice/contact bottom navigation was removed. Only `Existujúce faktúry` and `Existujúce kontakty` bind existing Stage A reads; all other sublevels show bounded local unavailable copy with zero hidden network/business effect.
- Preserved existing session, enrollment, workspace authority, sign-out confirmation, API route set, and Telegram selection isolation. Profile choice clears invoice/contact/detail/PDF state; each fresh ready shell starts at Home. No microphone permission, STT/LLM/LMM, Android business mutation, Telegram handler/FSM, DB/schema, server, Cloudflare, enrollment, or deployment change was added.
- Diagnosed the real Samsung invoice-detail defect to a concrete response-shape mismatch: Stage A and legacy storage permit `invoice_item.unit = null`, while Android required a non-null `String`. Changed only the Android response model to nullable with a safe UI fallback. Added a sanitized production-shaped MockWebServer fixture, same-workspace retry proof, and bounded not-available/network/protocol/unexpected detail states with safe retry/back controls.
- Reworked Home/list/detail/contact/unavailable/message surfaces around scrollable content, scaffold/system insets, wrapping column layouts, long active-profile copy, and reachable full-width actions. Added Compose instrumentation source for 1.3/1.5/2.0 font scale, long profile/contact text, and nullable-unit detail reachability; it compiled but was not executed locally because no emulator/device was attached.
- Synchronized `first_party_android_client` Product Truth and Slovak InfoHelp with current bounded evidence: the controlled HTTPS pilot is active and one authorized Samsung proved enrollment, authenticated session, multiple profiles, invoice listing, and contacts. C1 remains `partial`; the new Home build, repaired detail/PDF, restart, profile switch, responsive/back journey, and logout await real-device acceptance.
- Added `docs/evals/OFFICEFLOW_ANDROID_HOME_TOP_LEVEL_NAVIGATION_V2_acceptance_proof.md` with all 22 approved scenarios. Overall verdict remains `runtime_not_proven`, which is compatible with committing/publishing a controlled pilot APK but forbids claiming device acceptance.
- Validation: Android main/JVM/instrumentation sources compiled; `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug` passed; focused Product Truth/API/Android boundaries `45 passed`; full repository regression `2616 passed, 7 subtests passed in 448.05s`; `python -m compileall -q bot` and `git diff --check` passed.
- Next bounded step is the same Draft PR #105 pilot workflow build against `https://officeflow-pilot-api.zevsflow.sk`, artifact/checksum verification, and then real-device C1 acceptance. Merge, production deployment, server/Cloudflare/database changes, and additional enrollment issuance remain out of scope.
...
- Found one material protocol contradiction before implementation: Stage A returned indistinguishable 401 for ordinary temporary block and terminal revoked/deleted/expired sessions, while approved Stage B S25 requires block/unblock to remain non-terminal. After explicit user direction to continue, applied the smallest compatibility repair: protected access and refresh now return bounded 423 `access_temporarily_unavailable` only for current `blocked`; deleted/inactive/revoked/expired remains terminal 401. No route or schema was added.
- Added one `android/:app` project with application id `sk.zevsflow.officeflow`, minSdk 26, compile/target 35, Kotlin 2.1.20, AGP 8.8.2, Compose Material 3, Navigation Compose, ViewModel/coroutines, kotlinx.serialization, OkHttp, AndroidKeyStore AES/GCM, and `PdfRenderer`. Dependency wiring is manual; Firebase, Hilt/Dagger, Room, Retrofit, third-party PDF viewers, analytics, WebView, and logging interceptors are absent.
- Added administrator enrollment, encrypted no-backup credential persistence, a single Mutex-owned refresh coordinator, no blind replay after ambiguous rotation, local-only workspace selection validated against the latest server list, page-size-50 invoice projection, detail/items, bounded private PDF rendering/cleanup, contacts, and confirmed online/offline sign-out. Only the nine approved Stage A routes exist in the Android client.
- Product Truth now registers exactly `first_party_android_client` as `partial`, `requires_admin=true`, `requires_setup=true`; deterministic Slovak InfoHelp states the production API is not deployed and prohibits full/live/mutating/offline/public-signup claims. Capability answers remain answer-only.
- Added Android JVM tests for session/enrollment, concurrent refresh, ambiguity, workspace policy, approved routes/projections, PDF validation, and logout; Android instrumentation tests cover encrypted no-backup storage/corruption/logout and the fresh enrollment entry surface. Added Python boundary/auth/Product Truth/InfoHelp regressions, the Stage B acceptance proof, Gradle wrapper/readme, and a scoped GitHub Actions debug APK artifact workflow.
- Current acceptance verdict is `runtime_not_proven`: the implementation host has no emulator/device and the authored instrumentation smoke was not faked; the full Python suite also retains the unchanged clock-sensitive failure described here. GitHub Actions run `32309125355` passed Android JVM tests, lint, and `:app:assembleDebug`; artifact `officeflow-android-debug-b03b2812eba89f5395bfd35851be5f55b0894052` (id `9385867828`) contains the APK and checksum, and the downloaded APK matched SHA-256 `aded96b7cbf7db88c29b9ba771dec112699dd328ffe46478050df5d4561dcc0a`. Stage B/shared focused Python evidence is `95 passed`. Full regression is `2612 passed, 7 subtests passed, 1 failed`; the only failure reproduces unchanged in `test_explicit_close_now_closes_open_day` because the test opens a fixed day at 07:00 but calls the real-clock close path while the host time is earlier than 07:00. No work-time/FSM file was changed. `compileall` and `git diff --check` passed.
- Production migration, server access/change, API deployment/exposure, Cloudflare change, production enrollment, phone installation, Play Store publication, release signing, and merge are all **NO**.

# 2026-08-19 - OfficeFlow Stage A PR #104 merge-blocker repair

- Confirmed two actionable review blockers against the approved proof: the API PDF resolver accepted any file below the global invoice root, and administrator tooling could not revoke a lost-device session without its raw access token.
- Tightened `WorkspaceInvoicePdfStorageService.resolve_existing_read_path()` so a persisted pointer must resolve to the exact current workspace storage root and exact invoice filename. A numeric legacy owner root remains readable only when supplier/invoice ownership records map that owner to exactly one workspace. Flat, arbitrary, multi-workspace, symlink-resolved foreign, and poisoned workspace-A-to-workspace-B pointers fail closed with no path disclosure.
- Added the direct poisoned-pointer regression `workspace A invoice -> workspace B PDF`, plus numeric legacy single-workspace success and flat/multi-workspace legacy rejection coverage.
- Added safe administrator session inventory and revocation to `ApiSessionService` and `bot.cli.officeflow_api_access`: `sessions --telegram-id ...` returns only opaque session id, device label, timestamps, and bounded status; `revoke-session --telegram-id ... --session-id ...` is tenant-bound and idempotent. It exposes no principal, access/refresh credential, or token hash and can revoke a selected session even after the target user is blocked.
- Direct regression proves one lost-device session is revoked while another session for the same user and a foreign user's session remain untouched; wrong-user session id fails as not found. CLI output secrecy and post-revoke access/refresh rejection are covered.
- Preserved and implemented the concurrently approved `/vymazat_databazu` credential-revocation addendum. The existing exact-confirmed action now resolves the actor's retained active principal and, inside the same `BEGIN IMMEDIATE` transaction as business deletion and `deleted_database`, revokes all non-revoked sessions and pending enrollments. Missing principal remains a valid legacy case; unrelated principals are untouched; ordinary block/unblock does not terminalize credentials.
- Added regression evidence that pre-delete access/refresh/enrollment secrets remain invalid after re-approval, only a fresh administrator enrollment restores API use, database failure rolls back credential/business/access effects together, partial filesystem cleanup never resurrects credentials, and wrong/cancel/voice paths have zero credential effect. Existing warning/final copy now truthfully mentions first-party API credential invalidation without claiming an Android app.
- Final Stage A focused result: `40 passed in 20.68s`; existing account-reset/addendum suite: `12 passed in 13.27s`; combined targeted proof: `52 passed in 22.60s`; shared access/workspace/tenant regression: `64 passed`; final full repository regression: `2605 passed, 7 subtests passed in 176.81s`. `python -m compileall -q bot` and `git diff --check` passed.
- Architecture status remains unchanged: no new HTTP route, product identity, business action, FSM, AI/external call, schema, Android claim, production migration, or deployment.

# 2026-08-19 - OfficeFlow platform-neutral Android foundation V1 Stage A implementation

- Implemented the approved Stage A without material design variance. `workspace_id` remains the canonical tenant boundary; opaque `principal_id` is used only for server-owned platform-neutral identity and API-access infrastructure.
- Added the additive `principal`, `principal_external_identity`, `api_enrollment`, and `api_session` tables, required uniqueness/check constraints and lookup indexes, plus fail-closed bootstrap validation. No existing business table is rebuilt, renamed, or backfilled.
- Added lazy administrator-controlled Telegram-to-principal enrollment. The raw high-entropy enrollment secret is displayed once by `python -m bot.cli.officeflow_api_access issue`; status and revocation never expose secret hashes or principals. Blocked, deleted-database, or otherwise inactive users cannot issue, exchange, refresh, or use API access.
- Added opaque hashed access/refresh sessions with bounded expiries, persisted restart-safe lookup, current authorization checks, atomic credential rotation, replay rejection, current-session revocation, and bounded last-seen metadata. No JWT or client-asserted identity is used.
- Added the independently startable `python -m bot.officeflow_api_app` process with a focused API config that does not require `BOT_TOKEN`. The exact route set is enrollment exchange, session refresh/revoke/read, workspace list, invoice list/detail/PDF, and contacts. No action, conversation, voice, upload, workspace-switch, invoice/contact mutation, or mark-paid route exists.
- All business reads authenticate token -> session -> principal -> Telegram external identity -> current `AccessControlService` state -> current workspace membership -> requested scope -> workspace-owned read service. One membership may default per request without writing selection; multiple memberships require explicit scope and ignore persistent Telegram selection.
- Invoice/contact projections use explicit allowlists. Telegram compatibility ids, `pdf_path`, storage roots, contract/source paths, hashes, principals, and raw internal errors are not exposed. PDF reads require a persisted scoped invoice pointer resolving to an existing bounded `%PDF-` file inside the configured invoice artifact root; missing/unsafe/foreign paths fail as bounded not-found and never regenerate a PDF.
- Telegram polling, handlers, voice routing, active-FSM middleware, callbacks, keyboards, confirmation contracts, and business actions were not modified. API modules do not import aiogram, Telegram handlers, `bot.main`, STT/LLM/LMM, Semantic Action Resolver, InfoHelp, Gmail, Drive, or other external-service owners.
- Added `docs/evals/OFFICEFLOW_PLATFORM_NEUTRAL_ANDROID_FOUNDATION_V1_acceptance_proof.md`, mapping all 18 approved scenarios. A conversation proof is explicitly not applicable because Stage A adds no conversational action or FSM.
- Initial Stage A suite before the PR #104 blocker repair: `36 passed`. Existing shared access/workspace/tenant regression: `64 passed`. Initial full repository suite: `2598 passed, 7 subtests passed in 183.68s`. `python -m compileall -q bot` and `git diff --check` passed.
- Existing-data fixture bootstrap proves authorization, workspace ownership/membership/selection, supplier, contact, invoice/item relationships, and persisted PDF pointers remain unchanged; only the four approved tables and three lookup indexes are added. No production migration was run.
- Product Truth remains honest: protected first-party API/auth foundation exists for a controlled administrator pilot; business API is read-only; Android app and Android conversational/mutating workflows are not implemented; Telegram remains the current end-user runtime. No Android capability registry entry or InfoHelp runtime claim was added.
- Deployment status: code/test evidence only. Not merged, not deployed, no server backup/apply/restart, no `.env`/secret change, no Cloudflare/public route, and no production database write.

# 2026-08-19 - OfficeFlow platform-neutral Android foundation V1 Stage A preflight

- Approved proof: `docs/architecture/OFFICEFLOW_PLATFORM_NEUTRAL_ANDROID_FOUNDATION_V1_ARCHITECTURE_DESIGN_PROOF.md`, verdict `ready_for_handoff`.
- Repository preflight used exact `origin/main` HEAD `e5a4eb309d08ebd30ec96fef590bed3da6076aab`. The proof's reviewed code baseline `f1a56119da9b0ba5068f4e9060753f55c7de4afe` differs only by subsequent documentation/log changes; no material code/design contradiction was found.
- Contracts read: `AGENTS.md`, the approved proof, implementation/handoff/evaluation/top-level design contracts, Product Doctrine, Product Truth, TZ, multi-workspace proof, OfficeFlow framing, migration runbook, access roadmap, recent relevant project log, and the listed current code/test owners.
- Frozen constraints: `workspace_id` remains the business tenant; `principal_id` is additive internal identity; current Telegram compatibility columns remain; `authorized_users`/`AccessControlService` remains current access authority; enrollment and opaque sessions store only hashes; API is a separate bounded `aiohttp` process; all business endpoints are read-only and validate membership on every request; no active-workspace, FSM, Telegram, AI, Google, business-data, or Product Truth mutation is allowed.
- Existing owners: `AccessControlService`, `WorkspaceContextService`, `WorkspaceInvoiceService`, `WorkspaceContactService`, `ScopedInvoiceRuntime`, `WorkspaceInvoicePdfStorageService`, and `bot/services/db.py` bootstrap.
- Implementation path: add focused principal/enrollment/session/API-context/read-projection owners and administrator CLI; extend existing workspace invoice/PDF owners only where read projection is missing; keep HTTP handlers thin and independently startable without `BOT_TOKEN`.
- Persisted-data impact: four additive identity/API-access tables plus indexes only; no business-table rebuild/backfill/path rewrite. Local existing-data fixture tests will prove preservation. Production migration/apply, backup, restart, deploy, Cloudflare exposure, `.env`, and server access are explicitly out of scope.
- Access/tenant impact: protected requests follow access-token hash -> session -> principal -> Telegram external identity -> current active authorization -> current memberships -> requested workspace -> scoped owner. Client-supplied Telegram/principal identity is never accepted as authority; API reads never consult or write `active_workspace_selection`.
- Telegram/FSM impact: none planned; API reads remain independent and cannot import/execute handlers, clear/update FSM, invoke callbacks, send Telegram messages, or affect polling availability.
- AI boundary: zero STT/LLM/LMM/Semantic Action Resolver/InfoHelp calls; deterministic infrastructure only. AI maturity is unchanged.
- External-service/PDF impact: no external service calls and no PDF generation. PDF access is limited to persisted owned invoice pointers validated below the configured invoice storage root and streamed without path disclosure.
- Planned evidence: focused identity/enrollment/session/schema/API/tenant/PDF/negative-space/import-boundary tests, the relevant existing workspace/tenant regression set, `python -m pytest -q`, `python -m compileall -q bot`, `git diff --check`, and an 18-scenario task acceptance proof.
- Explicitly out of scope: Android app/UI, public signup, external identity providers, JWT, business mutations/actions/FSM, persistent workspace switching, uploads/voice/conversation/AI, notifications, production migration, deployment, merge, and any claim that Android is supported.
- Design variance status: `none_identified`.
