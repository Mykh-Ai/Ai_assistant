...
|---|---|---|---|---|---|
| Invoice period summary period selection (`current_year`, `previous_year`, `unknown`; explicit `YYYY` parsed by Python) | top-level action value group | implemented | mixed (text + voice after top-level action resolution) | `invoice_period_summary` handler calls bounded resolver context `invoice_summary_period_selection` after the top-level action token is selected; Python converts the canonical period token into exact `issue_date` range and executes tenant-scoped aggregation. | This is action-parameter canonicalization, not a separate top-level action and not a phrase whitelist. LLM may only choose from Python-provided period values; Python validates and executes. Unsupported periods such as month/quarter/custom ranges return safe guidance. |
