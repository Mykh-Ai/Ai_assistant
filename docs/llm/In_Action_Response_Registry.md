...
Accounting category UX note: current runtime may present Telegram reply keyboards for the accounting category preview, unknown-category decision, yes/no duplicate and new-category confirmations, similar-category warning, and reasonable category selection lists. These buttons send visible text back into the active FSM state; handlers still consume only canonical DecisionResolver outputs or Python-provided category ids, and terminal save/cancel/failure outcomes remove the custom keyboard.
