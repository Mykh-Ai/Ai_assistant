...
| `accounting_document_category_selection` | in-action value selection | partial | mixed (text + voice) inside category selection states | one Python-provided category id or `cancel` / `unknown` | `resolve_accounting_document_category_selection(...)` receives `allowed_categories` from Python and matches only category id/display label values from that list. | This is a bounded selection, not a top-level action. System and active workspace categories only are valid. |
