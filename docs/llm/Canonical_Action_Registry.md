...
| `show_supplier_profile` (working: `/moj_profil`) | top-level user-facing action | implemented | command | `cmd_moj_profil()` starts supplier profile creation when missing, or shows the current supplier profile summary when present. | User-facing profile command. `/supplier` remains legacy/technical onboarding alias. |
| `edit_supplier` (working: `/upravit_profil`) | top-level user-facing action | implemented | command + text in-flow | `cmd_upravit_profil()` starts a targeted one-field supplier profile edit; `supplier_profile_edit_confirm()` saves through `SupplierService.update_profile(...)`. | Uses Python validation per field and shared `yes_no` DecisionResolver context `supplier_profile_edit_confirm`. |
...
| `delete_user_database` (working: `/vymazat_databazu`, voice/text examples such as `Chcem vymazať moju databázu`) | top-level user-facing action | reserved | command + text + voice planned | User-facing approval text introduces the future database deletion command/intent. | Destructive hard-delete runtime is not implemented in this slice. Future implementation must show consequences and require exact manual confirmation `vymazať databázu`. |
| `show_recent_accounting_documents` (working: `/blocek`, legacy `/blocky`) | top-level user-facing action | implemented | command + deterministic text aliases | Dedicated accounting documents router reads confirmed accounting metadata through `accounting_document_registry`. | Read-only recent receipts/incoming accounting documents view. It lists only confirmed Document Intake metadata and is not a broad document browser, contract archive, invoice PDF view, temp upload view, search, edit, delete, or Google Drive sync action. |
| `add_receipt` (working: `/add_blocek`, `/dodat_blocek`) | top-level user-facing action | implemented | command | Existing accounting document intake command handler starts `AccountingDocumentIntakeStates.waiting_upload`. | User-facing entry for adding a new receipt/blocek. `/doklad` remains a broader legacy/reserved document-intake entry and is not promoted in `/start`. |
