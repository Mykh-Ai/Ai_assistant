...
| `show_supplier_profile` (working: `/moj_profil`) | top-level user-facing action | implemented | mixed (command + semantic text/voice) | `cmd_moj_profil()` starts supplier profile creation when missing, or shows the current supplier profile summary when present; semantic top-level routing maps `show_supplier_profile` to the same handler. | User-facing profile command. `/supplier` remains legacy/technical onboarding alias. |
| `edit_supplier` (working: `/upravit_profil`) | top-level user-facing action | implemented | mixed (command + semantic text/voice + text in-flow) | `cmd_upravit_profil()` starts a targeted one-field supplier profile edit; semantic top-level routing maps `edit_supplier` to the same handler; `supplier_profile_edit_confirm()` saves through `SupplierService.update_profile(...)`. | Uses Python validation per field and shared `yes_no` DecisionResolver context `supplier_profile_edit_confirm`. Exact field values remain text-first. |
