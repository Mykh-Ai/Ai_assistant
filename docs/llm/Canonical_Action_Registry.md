...
| `add_service_alias` (working: `/service`) | top-level user-facing action | implemented | command (+ text inside flow) | `/service` command starts two-step alias mapping flow in supplier handler; writes mapping via `ServiceAliasService.create_mapping(...)`. | **Implemented manual action.** Voice top-level invoke: not yet. Ambiguous action: yes. For future bounded top-level resolution, semantic hint support is recommended. Canonical Slovak-facing wording: `pridaj novú položku`, `pridaj novú službu`. |
