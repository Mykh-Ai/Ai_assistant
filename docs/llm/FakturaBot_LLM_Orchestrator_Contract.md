...
For an invoice analytics question that explicitly names one customer, Python may run bounded context `invoice_analytics_customer_resolution` over unique current-tenant contact names plus `unknown`. A selected contact is enforced by Python as a trusted `contact_id` dataframe prefilter before planning; the planner must not apply a second raw-name/contact-id filter. This read-only resolution does not create a contact alias or mutate invoice history.
