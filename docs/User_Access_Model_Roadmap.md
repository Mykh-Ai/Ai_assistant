...
- Narrow maintenance exception: when the separately gated periodic contact-registry monitor is enabled, it may read official data and create a confirmation proposal for a persisted inactive workspace/membership only while the supplier owner remains an active authorized user. It does not reactivate or expose the profile; contact writes remain button-confirmed. Same-owner duplicate contacts may be grouped by canonical IČO and identical official target snapshot across that actor's authorized workspaces, but another actor is never included and every grouped workspace is revalidated before one atomic write transaction.
