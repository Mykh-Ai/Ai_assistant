...
AI не є джерелом істини. AI використовується як помічник для побудови чернетки.

Усі критичні сценарії працюють за моделлю:
...
**text → LLM/parser → draft → confirmation**
...
### 5.1 Де AI справді потрібен

AI потрібен для:
- розуміння живого диктування,
- нормалізації коротких або побутових фраз,
- витягу реквізитів із договору,
- побудови структурованої чернетки фактури.

### 5.2 Де AI не має приймати остаточне рішення

AI не може безконтрольно:
- створювати фактуру без перегляду,
- зберігати контрагента без підтвердження,
- вибирати сторону договору без валідації,
- підміняти реквізити постачальника й замовника,
- вигадувати відсутні поля.

### 5.3 Нормалізація коротких назв робіт

У користувача можуть бути власні короткі або побутові позначення.
...
Бот повинен уміти через AI та prompt/context приводити це до правильного формулювання для фактури.

Наприклад:
- `оправи` → `Opravy vyhradených technických zariadení elektrických`

### 5.4 Персоналізація через prompt

У майбутньому під кожного користувача або клієнта може існувати власний prompt/context, у якому описано:
- типові формулювання,
- короткі назви робіт,
- бажаний стиль запису позицій,
- термінологію конкретної професії.

Це одна з причин, чому проєкт краще розвивати як кастомізований інструмент, а не як жорсткий масовий SaaS.
...
Будь-яка фактура перед створенням PDF повинна показуватись як чернетка:
- контрагент,
- позиція,
- кількість,
- сума,
...
- системний промт,
- текст договору,
- інструкцію знайти саме замовника,
- вимогу повернути тільки JSON.
