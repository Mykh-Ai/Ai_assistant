...
The authorized read-only audit on 2026-07-18 verified production mappings without logging raw taxpayer values: `ds_dsrdp` searches `ico` and returns `dic`; `ds_dphs` searches `ico` and directly returns `ic_dph`; both use the documented page envelope and return 404 for no exact result. `verified_financna_sprava_schema()` binds only those audited lowercase fields. This local provider/config/FSM integration remains disabled by default and is not a claim that production enrichment is deployed or active.
