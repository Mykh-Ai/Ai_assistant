...
- configured admins may use `/approve <telegram_id>`, argument-free self-target `/approve`, `/reject <telegram_id>`, `/block <telegram_id>`, and `/users`;
...
## 2026-07-18 Addendum: registry search quality and staged tax enrichment

RPO name results are normalized and ranked deterministically in Python. Legal-suffix punctuation/spacing variants share a canonical suffix representation. One exact active normalized name suppresses lower weak matches and opens detail; several exact entities remain selectable with IČO and municipality. Whole-token and bounded partial suggestions remain limited by the configured maximum and always require user selection. Compact spacing or one-edit suggestions may surface `ZE VS` -> `Zevs` and `Empbau` -> `Empebau`/`Empe bau`; they are not exact identities. Internal substrings such as `zevs` inside `Klimaszevská` are rejected.

`bot/services/slovak_tax_registry.py` owns the separate asynchronous Financial Administration boundary and `SlovakCompanyDetailsAggregator` combines it with RPO only after one exact RPO IČO is selected. The provider validates exact returned IČO, DIČ, and IČ DPH, rejects name-only/wrong-IČO/invalid/conflicting rows, never creates `SK + DIČ`, performs no candidate-list or final-confirmation requests, stores no raw response, and maps failures to bounded codes. Tax failure preserves the RPO draft and typed-DIČ continuation.

Configuration is `CONTACT_TAX_LOOKUP_ENABLED=0`, `FINANCNA_SPRAVA_API_KEY=`, and a positive `FINANCNA_SPRAVA_TIMEOUT_SECONDS` capped at 30. The RPO feature/pilot gate remains the parent. The API key is hidden from config representation and must not enter logs, exceptions, FSM/callback data, previews, tests, docs examples, or persistence. The production base URL is fixed in code; tests inject fake schema/base behavior and never call the internet.

The authorized read-only audit on 2026-07-18 verified production mappings without logging raw taxpayer values: `ds_dsrdp` searches `ico` and returns `dic`; `ds_dphs` searches `ico` and directly returns `ic_dph`; both use the documented page envelope and return 404 for no exact result. `verified_financna_sprava_schema()` binds only those audited lowercase fields. The controlled server pilot was deployed and enabled on 2026-07-18 behind the existing RPO workspace gate; this is not general availability, and controlled Telegram acceptance remains pending.
...
A detected official name, legal-address, DIČ, or IČ DPH difference is notification-only until the owner chooses `Aktualizovať kontakt`. Approval revalidates actor, every explicitly grouped workspace, TTL, canonical IČO, contact version/old values, and conflicts in one immediate transaction. Decline, replay, expiry, unauthorized actor, stale contact, ambiguity, and conflicts fail closed without a partial group write.

Every persisted proposal remains bound to one saved contact snapshot and opaque UUID. Proposals for different actors, canonical IČO values, or official target snapshots remain independently actionable. Duplicate contacts for the same authorized owner, formatting-equivalent IČO, and identical official target snapshot receive one grouped card with the affected profile count; one confirmation atomically updates all grouped rows. Owned stale, expired, and identity-conflict outcomes remove their obsolete buttons and explain the bounded reason; missing or forbidden callbacks do not alter markup because ownership is unproven. Cleanup failure is logged without rolling back an already committed contact update.
