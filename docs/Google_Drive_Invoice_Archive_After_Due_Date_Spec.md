...
```text
FakturaBot/<workspace.drive_folder_name>/<issue-year>/faktury/
```
...
profile selection. All outgoing invoices for one profile and issue year share
that annual `faktury` folder; invoice archive paths do not create month
subfolders. Legacy invoices whose persisted `workspace_id` is null retain the
same year/type shape without a workspace segment.
