...
def test_report_includes_all_days_sundays_and_total(tmp_path: Path) -> None:
    ...
    assert report.report_path.name == 'dochadzka_2026_06.xlsx'
    workbook = load_workbook(report.report_path)
    sheet = workbook.active
    assert sheet.max_row == 34  # title, blank, header, 30 days, total
    assert sheet['A1'].value == 'Vykaz odpracovanych hodin za jun 2026'
    assert sheet['D18'].value == '11:30'
    assert sheet['D34'].value == '11:30'
    assert sheet['A10'].fill.fgColor.rgb in {'00C6EFCE', 'C6EFCE'}
