...
def test_contact_registry_config_defaults_are_disabled_and_bounded() -> None:
    ...
    assert fields['contact_tax_lookup_enabled'].default is False
    assert fields['financna_sprava_api_key'].default is None
    assert fields['financna_sprava_timeout_seconds'].default == 5
...
def test_info_help_explains_registry_contact_support_and_limits() -> None:
    ...
    assert 'predvolene vypnuté' in answer
    assert 'API kľúč' in answer
